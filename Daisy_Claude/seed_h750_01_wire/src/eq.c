// eq.c — EQ filter coefficient computation and sample processing
//
// High-shelf formula from the Audio EQ Cookbook (R. Bristow-Johnson), shelf
// slope S=1. Coefficients are normalised by a0 and converted to the
// positive-feedback storage convention used by Biquad (a1/a2 are negated
// relative to the cookbook's "subtract" sign).
//
// 1-pole LP uses the matched-Z / leaky-integrator form:
//   alpha = exp(-2π*fc/fs),  b0 = 1-alpha,  a1 = alpha

#include <math.h>
#include "eq.h"
#include "params.h"
#include "audio.h"

EqChannel eq_ch[2];

static float prev_shelf_gain[2];
static float prev_shelf_fc[2];
static float prev_lp_fc[2];

#define PI_F  3.14159265358979323846f

static void compute_hishelf(Biquad *bq, float gain_db, float fc_hz)
{
    // A = 10^(dBgain/40); w0 in radians; alpha = sin(w0)/sqrt(2) for S=1
    float A     = powf(10.0f, gain_db * 0.025f);
    float w0    = 2.0f * PI_F * fc_hz / AUDIO_SAMPLE_RATE;
    float sw0   = sinf(w0);
    float cw0   = cosf(w0);
    float alpha = sw0 * 0.70710678f;            // sin(w0) / sqrt(2)
    float sqA   = __builtin_sqrtf(A);  // VSQRT.F32 — no libm wrapper needed
    float Am1   = A - 1.0f;
    float Ap1   = A + 1.0f;
    float tsa   = 2.0f * sqA * alpha;            // 2*sqrt(A)*alpha

    float b0    =  A * (Ap1 + Am1 * cw0 + tsa);
    float b1    = -2.0f * A * (Am1 + Ap1 * cw0);
    float b2    =  A * (Ap1 + Am1 * cw0 - tsa);
    float a0    =       Ap1 - Am1 * cw0 + tsa;
    float a1_ck =  2.0f * (Am1 - Ap1 * cw0);   // cookbook sign: subtract
    float a2_ck =         Ap1 - Am1 * cw0 - tsa;

    float inv_a0 = 1.0f / a0;
    bq->b0 =  b0    * inv_a0;
    bq->b1 =  b1    * inv_a0;
    bq->b2 =  b2    * inv_a0;
    bq->a1 = -a1_ck * inv_a0;  // negate: store for positive-feedback form
    bq->a2 = -a2_ck * inv_a0;
}

static void compute_lp(OnePoleLP *lp, float fc_hz)
{
    float alpha = expf(-2.0f * PI_F * fc_hz / AUDIO_SAMPLE_RATE);
    lp->b0 = 1.0f - alpha;
    lp->a1 = alpha;
}

float eq_process_biquad(Biquad *bq, float x)
{
    float y = bq->b0 * x
            + bq->b1 * bq->x1 + bq->b2 * bq->x2
            + bq->a1 * bq->y1 + bq->a2 * bq->y2;
    bq->x2 = bq->x1;  bq->x1 = x;
    bq->y2 = bq->y1;  bq->y1 = y;
    return y;
}

float eq_process_lp(OnePoleLP *lp, float x)
{
    float y = lp->b0 * x + lp->a1 * lp->y1;
    lp->y1 = y;
    return y;
}

void eq_update_from_params(void)
{
    for (int ch = 0; ch < 2; ch++) {
        float sg = eq_params[ch].shelf_gain->value;
        float sf = eq_params[ch].shelf_fc->value;
        float lf = eq_params[ch].lp_fc->value;

        if (sg != prev_shelf_gain[ch] || sf != prev_shelf_fc[ch]) {
            compute_hishelf(&eq_ch[ch].hi_shelf, sg, sf);
            prev_shelf_gain[ch] = sg;
            prev_shelf_fc[ch]   = sf;
        }
        if (lf != prev_lp_fc[ch]) {
            compute_lp(&eq_ch[ch].lp, lf);
            prev_lp_fc[ch] = lf;
        }
    }
}

void eq_init(void)
{
    for (int ch = 0; ch < 2; ch++) {
        compute_hishelf(&eq_ch[ch].hi_shelf,
                        eq_params[ch].shelf_gain->value,
                        eq_params[ch].shelf_fc->value);
        compute_lp(&eq_ch[ch].lp, eq_params[ch].lp_fc->value);

        prev_shelf_gain[ch] = eq_params[ch].shelf_gain->value;
        prev_shelf_fc[ch]   = eq_params[ch].shelf_fc->value;
        prev_lp_fc[ch]      = eq_params[ch].lp_fc->value;
    }
}

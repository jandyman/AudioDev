...
| ⚠ Open input-topology proposal (`adc-netlist.md` §2.1): DC-coupled differential inputs on one added cable conductor, deleting 24 parts from the codec section | Decide at the netlist review gate, not after layout — it changes the connector, the cable and the codec input area. Blocking check is the converter's DC-coupled common-mode window; until then lay the section out with §2.1's retrofit footprints so either configuration can be fitted |
| ⚠ Documentation divergence between `documentation/` and the preamp project's own notes | The reference-architecture change of 2026-08-22 reached `documentation/` only on 2026-08-26. Cross-reference tables in each document now list the preamp project's notes; check them before treating any analog section as current |

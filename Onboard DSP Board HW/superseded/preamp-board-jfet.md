...
>
> The per-string front end is now a low-noise operational amplifier with gain.
> The current design is `../preamp-board.md`; the reasoning for the change, the
> measurements behind it, and the conditions under which this design would
> return are in `../analog-front-end.md`.
>
> **This document is preserved unedited and remains useful for one thing:** its
> §2 is the only device characterisation of the source-follower operating point
> — the Idss and gate-source spread analysis — which any future JFET-based
> front end would need.

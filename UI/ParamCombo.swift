...
@IBDesignable open class ParamCombo: UIControl {
    ...
    @objc func sliderChanged(_ sender: AnyObject?) {
        ...
        valueBox.text = String(format: fmtStr, mapped)
        sendValueChanged()
    }
    ...
    func commonInit() {
        ...
        let mask: UIControlEvents = [UIControlEvents.editingDidEndOnExit, UIControlEvents.editingDidEnd];
        valueBox.addTarget(self, action: #selector(textChanged(_:)), for: mask)
        valueBox.keyboardType = UIKeyboardType.decimalPad
        valueBox.borderStyle = UITextBorderStyle.roundedRect
...
@IBDesignable open class NumericCombo: UIControl {
    ...
    @objc func stepperChanged(_ sender: AnyObject?) {
        valueBox.text = String(format: "%1.0f", stepper.value)  // tmp kludge
        sendValueChanged()
    }
...
@IBDesignable open class NumericCombo2: UIControl {
    ...
    func commonInit() {
        ...
        btnDown.Color = Color
        btnUp.Color = Color
        btnDown.addTarget(self, action: #selector(btnDnTouchDown(_:)), for: UIControlEvents.touchDown)
        btnDown.addTarget(self, action: #selector(btnDnTouchUp(_:)), for: UIControlEvents.touchUpInside)
        btnUp.addTarget(self, action: #selector(btnUpTouchDown(_:)), for: UIControlEvents.touchDown)
        btnUp.addTarget(self, action: #selector(btnUpTouchUp(_:)), for: UIControlEvents.touchUpInside)
        addSubview(valueBox)
        addSubview(btnUp)
        addSubview(btnDown)
    }

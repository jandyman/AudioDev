...
def emit(graph, registry, order, inputs_of, instances, out_path, graph_src_path):
  ...
  L('};')
  L('')
  # Canonical handle: generic host code (pybind template, firmware runner) refers
  # to `audio_graph` and never needs to know the specific graph's class name.
  L(f'using audio_graph = {class_name};')
  L('')
  ...
  write_manifest(graph, registry, out_path, graph_src_path)


def write_manifest(graph, registry, out_path, graph_src_path):
  """Emit a <module>.deps.mk fragment next to the header: the hand-written C++
  block sources + their header dirs, so downstream builds (native make, firmware
  make) derive the block set from the .graph instead of re-declaring it. Absolute
  paths — regenerated every build (build/ is gitignored), so portability is moot.
  Faust blocks are NOT listed: their generated .cpp is #included by the header."""
  graph_dir = os.path.dirname(os.path.abspath(graph_src_path))
  seen = set()
  cpp_sources = []
  inc_dirs = [graph_dir]
  for b in graph['blocks']:
    t = b['type']
    defn = registry[t]
    if defn['is_faust'] or t in seen:
      continue
    seen.add(t)
    src = os.path.abspath(defn['source_file'])
    cpp_sources.append(src)
    # Header lives next to the .cpp (project-local block) or in a sibling
    # include/ (dsp_cpp's src/ + include/ split). Record whichever exists.
    d = os.path.dirname(src)
    for h in (os.path.join(d, t + '.h'),
              os.path.join(os.path.dirname(d), 'include', t + '.h')):
      if os.path.exists(h):
        inc_dirs.append(os.path.dirname(h))
        break
  inc_dirs = list(dict.fromkeys(inc_dirs))   # dedup, keep order

  man_path = (out_path[:-2] if out_path.endswith('.h') else out_path) + '.deps.mk'
  with open(man_path, 'w') as f:
    f.write(f"# Generated by graph_compiler.py from {os.path.basename(graph_src_path)} — DO NOT EDIT.\n")
    f.write("# Absolute paths; regenerated each build. Faust blocks are #included by the header.\n")
    f.write(f"GRAPH_NAME := {graph['name']}\n")
    f.write("GRAPH_CPP_SOURCES := " + " ".join(cpp_sources) + "\n")
    f.write("GRAPH_BLOCK_INCLUDE_DIRS := " + " ".join(inc_dirs) + "\n")

...
# ============================================================
# Documentation assembly  (--doc mode)
# ============================================================
#
# Concatenates a per-graph intro (`<graph>.intro.md`, sibling of the .graph)
# and each block's sibling `<type>.md` (next to its source) in topological
# order into build/generated/<graph>.md. The block's I/O line is generated
# from the @block marker so it never drifts; the prose lives in the .md.

def block_doc_path(defn):
  """Sibling `<type>.md` next to the block's source file."""
  d = os.path.dirname(defn['source_file'])
  stem = os.path.splitext(os.path.basename(defn['source_file']))[0]
  return os.path.join(d, stem + '.md')

def emit_doc(graph, registry, order, instances, out_path, graph_src_path):
  name = graph['name']
  ...
  L(f'<!-- Generated by graph_compiler.py from {os.path.basename(graph_src_path)} '
    f'+ sibling block .md files. DO NOT EDIT — edit the intro / per-block .md. -->')
  L('')

  intro_path = os.path.join(graph_dir, name + '.intro.md')
  if os.path.exists(intro_path):
    with open(intro_path) as f:
      L(f.read().rstrip())
  else:
    L(f'# {name}')
    print(f"warning: no graph intro '{intro_path}'")
  L('')

  for inst in order:
    defn = instances[inst]
    typ  = defn['name']
    kind = 'Faust' if defn['is_faust'] else 'C++'
    ins  = ', '.join(f'`{p}`' for p in defn['inputs'])  or '—'
    outs = ', '.join(f'`{p}`' for p in defn['outputs']) or '—'
    L('---')
    L('')
    L(f'## `{inst}` — {typ} ({kind})')
    L('')
    L(f'**inputs:** {ins} &nbsp;·&nbsp; **outputs:** {outs}')
    L('')
...
def main():
  ...
  if args and args[0] == '--doc':
    ...
    args = args[1:]
  if len(args) != 2:
    print("Usage: graph_compiler.py [--doc] <graph.graph> <out.h|out.md>", file=sys.stderr)
    sys.exit(1)
  graph_path, out_path = args
  ...
  if doc_mode:
    emit_doc(graph, registry, order, instances, out_path, graph_path)
  else:

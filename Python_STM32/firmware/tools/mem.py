...
debugger reading SRAM directly.  A build-ID handshake (audio_build_id vs the
.buildid sidecar) ensures a stale .map can't resolve wrong addresses.
...
def read_buildid(buildid_file):
  with open(buildid_file) as f:
...
def check_build_id(mem, buildid_file):
  """Abort unless the running firmware's build id matches the .buildid sidecar."""
  expected = read_buildid(buildid_file)
  target = mem.read_u32(mem.sym('audio_build_id'))
  if target == 0:
    raise RuntimeError("audio_build_id == 0 — firmware not booted past init / not running")
  if target != expected:
    raise RuntimeError(
      f"build-ID mismatch: target 0x{target:08x} != .buildid 0x{expected:08x}\n"
      "  → flashed firmware ≠ this .map; reflash the firmware or rebuild the host artifacts")

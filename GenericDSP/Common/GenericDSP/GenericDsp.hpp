...
struct WireSpec {
  ...
  struct PinSpec {
    ...
    struct DesignContext {
      ...
      Wire* AddConnectionWire() { return AddWire(connectionWires); }
      ...
      void Describe(bool connectionMode) {
        ...
        cout << "Wires:" << "\n";
        for (auto& wire : wires) { DescribeWire(wire, connectionMode); }
        if (processing_order.size() > 0) {
          ...
          for (auto& blk : processing_order) {
            for (int i=0; i < blk->nOutputPins(); i++) {
              ...
              DescribeWire(*pin.wire, connectionMode);
            }
          }
        }
      }
    ...
    struct GraphBase: DspInterface {
      ...
      void Connect(DspInterface* src, int srcPinIdx, DspInterface* dst, int dstPinIdx) {
        ...
        auto srcWire = designContext.DesignWireForOutputPin(srcPin);
        if (designContext.DesignWireForInputPin(dstPin) != nullptr) {
    ...
    struct TopLevelGraph : GraphBase {
      ...
      vector<PinSpec> FlattenDestinationNet(Pin& source) {
        vector<PinSpec> dsts;
        auto sourceWire = designContext.DesignWireForOutputPin(source);

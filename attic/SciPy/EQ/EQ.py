...
from scipy.signal import bilinear, freqz, lfilter, freqs
...
def freq_plot_helper(freqs: ArrayLike, resp: ArrayLike,
                     minDb=-20, maxDb=20):
...
def freq_plot_db(tf, Fs, minDb=-20, maxDb=20, impulseResponse=False):
  ...
  if impulseResponse:
    ...
    resp = resp[1]
  freq_plot_helper(freqs, resp, minDb=minDb, maxDb=maxDb)
...
def freqs_plot_db(tf, minDb=-20, maxDb=20):
  ...
  freq_plot_helper(resp[0] / (2 * pi), resp[1], minDb=-20, maxDb=20)

  
def impulse(n, dtype=np.float32):
  return append(array(1.0, dtype=dtype), zeros(n-1,dtype=dtype))
